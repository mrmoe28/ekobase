...
import {
  ...
  type ColumnInfo, type SchemaMap, type TableData,
} from '@/lib/api'
...
export default function TableEditorPage() {
  ...
  const [expandedSchemas, setExpandedSchemas] = useState<Set<string>>(new Set(['public', 'auth', 'admin']))
  ...
  const loadRows = useCallback(async (schema: string, table: string, off: number) => {
    setLoadingData(true)
    try {
      const data = await getTableRows(schema, table, PAGE_SIZE, off)
      setTableData(data)
    } catch {
  ...
  const selectTable = (schema: string, table: string) => {
    ...
    setOffset(0)
    setTableData(null)
    loadRows(schema, table, 0)
  }

  const goPage = (newOffset: number) => {
    if (!selected) return
    setOffset(newOffset)
    loadRows(selected.schema, selected.table, newOffset)
  }

  const columns = selected && schemaMap ? (schemaMap[selected.schema]?.[selected.table] ?? []) : []
  ...
  const handleDelete = async (row: Record<string, unknown>) => {
    ...
    setDeletingRow(row)
    try {
      await deleteTableRow(selected.schema, selected.table, buildPk(row))
      setTableData(prev => prev
        ? { ...prev, rows: prev.rows.filter(r => r !== row), total: prev.total - 1 }
        : prev)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updated = await updateTableRow(selected.schema, selected.table, pk, data)
      setTableData(prev => prev
        ? { ...prev, rows: prev.rows.map(r => r === editingRow ? updated : r) }
        : prev)
      setEditingRow(null)
  ...
  return (
    <div className="flex gap-0 -mx-6 -my-6 lg:-mx-8 lg:-my-8" style={{ height: 'calc(100svh - 4rem)' }}>
      {/* ── Schema tree ───────────────────────────────────────────────── */}
      <aside className="w-60 shrink-0 overflow-y-auto border-r flex flex-col"
        style={{ borderColor: 'var(--border)', backgroundColor: 'var(--sidebar-bg)' }}>
        <div className="px-4 py-3 border-b flex items-center gap-2"
          style={{ borderColor: 'var(--border)' }}>
          <Table2 size={15} style={{ color: 'var(--accent)' }} />
          <span className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Tables</span>
          {loadingSchema && <Loader2 size={13} className="animate-spin ml-auto" style={{ color: 'var(--text-muted)' }} />}
        </div>

        <div className="py-1 flex-1">
          {schemaMap && Object.entries(schemaMap).map(([schema, tables]) => (
            <div key={schema}>
              <button
              ...
              {expandedSchemas.has(schema) && Object.keys(tables).map(table => {
          ...
          <>
            ...
            <div className="flex items-center gap-3 px-4 py-2.5 border-b shrink-0"
              ...
              <span className="text-sm font-medium font-mono" style={{ color: 'var(--text)' }}>
                {selected.schema}.<strong>{selected.table}</strong>
              </span>
              {tableData && (
              ...
              <button
                onClick={() => loadRows(selected.schema, selected.table, offset)}
            ...
            {columns.map(col => (
              <div key={col.column}>
                <label className="label flex items-center gap-1.5">
                  ...
                  {col.is_pk && <span className="text-xs px-1 rounded" style={{ backgroundColor: 'color-mix(in srgb, var(--accent) 15%, transparent)', color: 'var(--accent)' }}>PK</span>}
                </label>
                <input
