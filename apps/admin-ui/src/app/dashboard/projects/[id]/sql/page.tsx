...
export default function ProjectSqlPage() {
  ...
  const historyKey = `sql_history_proj_${id}`

  const [query, setQuery] = useState(`SELECT * FROM ${projectSchema}.your_table LIMIT 10;`)
  ...
  const runQuery = useCallback(async () => {
    ...
    try {
      ...
      const entry: HistoryEntry = {
        id: crypto.randomUUID(), query: query.trim(),
        timestamp: Date.now(), success: true, rowCount: res.rowCount,
      }
      setHistory(prev => { const next = [entry, ...prev]; saveHistory(historyKey, next); return next })
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Query failed'
      setError(msg)
      const entry: HistoryEntry = {
        id: crypto.randomUUID(), query: query.trim(),
        timestamp: Date.now(), success: false, rowCount: null,
      }
      setHistory(prev => { const next = [entry, ...prev]; saveHistory(historyKey, next); return next })
    } finally {
      setRunning(false)
    }
  }, [query, running, historyKey])
    ...
    <div className="max-w-6xl mx-auto space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
            Schema: <code className="font-mono">{projectSchema}</code> · Ctrl+Enter to execute
          </p>
        </div>
      </div>
      ...
      <div className="card" style={{ border: '1px solid var(--border)' }}>
        <textarea
          ...
          placeholder={`SELECT * FROM ${projectSchema}.your_table LIMIT 10;`}
        />
        <div className="flex items-center justify-between px-4 py-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={runQuery} disabled={running || !query.trim()} className="btn-primary px-4 py-1.5 text-xs">
      ...
      <div className="card" style={{ border: '1px solid var(--border)' }}>
        ...
        {historyOpen && (
          <div style={{ borderTop: '1px solid var(--border)' }}>
            {history.length === 0 ? (
              <p className="px-4 py-4 text-sm text-center" style={{ color: 'var(--text-muted)' }}>No history yet</p>
            ) : (
              <>
