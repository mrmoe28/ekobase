...
import { listSecrets, upsertSecrets, deleteSecret, listProjectSecrets, upsertProjectSecrets, deleteProjectSecret, type FunctionSecret } from '@/lib/api'
...
export default function FunctionSecretsManager({
  ...
  emptyStateTitle = 'No custom secrets created',
  emptyStateDescription = 'This instance has no custom secrets yet.',
  projectId,
}: {
  title?: string
  description?: string
  customSecretsDescription?: string
  ...
  const load = async () => {
    ...
    try {
      if (projectId) {
        setSecrets(await listProjectSecrets(projectId))
      } else {
  ...
  const handleSave = async () => {
    ...
    try {
      if (projectId) {
        await upsertProjectSecrets(projectId, valid.map(r => ({ name: r.name.trim(), value: r.value })))
      } else {
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{customSecretsDescription}</p>
          </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center" style={{ backgroundColor: 'var(--surface)' }}>
                    <p className="text-sm font-medium" style={{ color: 'var(--text)' }}>{emptyStateTitle}</p>
                    <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>{emptyStateDescription}</p>
                  </td>
                </tr>
              ) : filtered.map(s => (
